...
alter table public.agent_inbox_items
  add constraint agent_inbox_items_kind_check check (kind in (
    ...
    -- (migration 0175) Agente publicado e ativo sem responder ninguém — versão
    -- publicada sem credencial vinculada, ou tráfego real recente sem nenhuma
    -- atividade correspondente (ai_agent_runs/ai_invocations). Achado auditando
    -- o CHANGELOG do fornecedor (mesma classe de bug que eles corrigiram) e
...
-- ---- Dashboard de Vendas, Fase 1: fn_sales_dashboard (migration 0155) ----
-- KPIs + gráficos com dado que já existe em crm_leads (value_cents, closed_at,
-- created_at, status, source) — sem coluna nova. SECURITY INVOKER igual 0037/
-- 0133: a RLS de crm_leads escopa dentro da função (agent vê só os próprios
-- leads; manager+ vê a org inteira). Janela semiaberta [p_from,p_to); duas
-- métricas de data (created_at pra "criados", closed_at pra "vendas") sem
-- coorte — mesma simplificação do relatório de referência que motivou a
-- feature. Aditiva e idempotente. Fica ANTES da varredura de anon logo abaixo
-- (função criada depois dela nasceria exposta — tests/unit/varredura-anon-e-
-- -o-ultimo-bloco.test.ts reprova isso estaticamente).
...
-- ---- Dashboard de Vendas ganha filtro de pipeline e origem (migration 0156) ----
-- Assinatura muda (2 parâmetros novos, default null = sem filtro): drop
-- explícito da versão de 3 parâmetros antes do create, senão fica sobrecarga
-- órfã. Mesmos filtros aplicados às três consultas que já liam crm_leads.
...
-- ---- Dashboard de Vendas, Fase 2: Receita por Serviço + Objeções (migration 0157) ----
-- Assinatura não muda — só o corpo do retorno jsonb ganha dois blocos novos,
-- create or replace puro. receita_por_servico agrupa a MESMA CTE base por
-- custom_fields->>'produto_interesse' (select configurável em
-- crm_pipelines.settings.service_options, sem coluna nova). principais_objecoes
-- lê crm_lead_activities type='objection' (vocabulário aberto, sem CHECK na
-- tabela) agrupado por payload->>'code'.
...
-- ---- Dashboard de Vendas, Fase 3: Funil real — Agendamento → Presença → Conversão (migration 0158) ----
--
...
-- ---- Meta Ads, Fase C1: credenciais por tenant + log do Purchase automático (migration 0159) ----
--
...
-- ---- Meta Ads, Fase D: receita por anúncio no Dashboard de Vendas (migration 0160) ----
--
-- A Fase A já captura source_metadata.ad_id/ad_headline no nascimento do
-- lead — esta fase só agrupa o que já existe, mesmo padrão de `servico`
-- (migration 0157): reaproveita a MESMA CTE `base`, sem consulta nova.
-- Agrupa por ad_id (estável), mostra o headline mais recente visto e cai
-- pro próprio ad_id sem headline. Lead sem atribuição de anúncio fica de
-- fora por definição.
create or replace function public.fn_sales_dashboard(
  p_org uuid,
  p_from timestamptz,
...
-- ---- Meta Ads, Fase E1: hierarquia de campanha/conjunto/anúncio (migration 0161) ----
--
-- Duas peças novas: `tenant_meta_ads_credentials.ads_read_token_encrypted`
-- (segundo token, INDEPENDENTE do access_token_encrypted da Fase C1 — o
-- token gerado pelo Gerenciador de Eventos só tem `read_ads_dataset_quality`,
-- não `ads_read`/`ads_management`, e reconectar arriscaria quebrar o
-- Purchase que já funciona; token novo e opcional é estritamente aditivo) e
-- `meta_ads_ad_metadata` (cache ad_id -> campanha/conjunto, resolvido em
-- segundo plano pela Fase E2, nunca no caminho do webhook).
...
-- Só o service_role escreve (handler roda com admin client, bypassa RLS de
-- qualquer forma) — sem policy de escrita pro usuário autenticado, mesmo
-- padrão de meta_capi_send_log (migration 0159).
...
-- View segura: mesma lista de colunas + um booleano derivado, nunca o token
-- em si.
create or replace view public.tenant_meta_ads_credentials_safe as
select
  id,
...
-- segundo plano; esta fase só LÊ o que já está cacheado, via LEFT JOIN —
-- nenhuma chamada de rede aqui. `receita_por_anuncio` MANTÉM o nome (não
-- virou `desempenho_por_anuncio`) — aditiva, o front-end já lê essa chave.
-- `agendamentos` por anúncio usa EXISTS correlacionado (não JOIN direto) pra
-- não multiplicar linha de `base` quando um lead tem mais de um
-- `meeting_scheduled` (remarcação, doutrina da 0158).
create or replace function public.fn_sales_dashboard(
  p_org uuid,
  p_from timestamptz,
  p_to timestamptz,
...
-- ---- "Cliente já existente", Fase 5: leads_total exclui existing_customer + LTV/recompra (migration 0166) ----
--
-- Fecha o roteiro (Fase 1 schema, Fase 2 automático, Fase 3 manual, Fase 4
-- some do board). `leads_total`/`leads_por_dia`/`receita_por_origem`/
-- `servico`/`anuncio` agora EXCLUEM status='existing_customer' — o problema
-- original que motivou a feature inteira: reconhecer alguém como cliente de
-- antes não pode inflar "quantos leads entraram este mês". `status='won'`
-- nunca coincide com `existing_customer` (desfechos mutuamente exclusivos),
-- então excluir de `base` não tira nada do lado de vendas/receita.
--
-- `ltv_medio_cents`/`taxa_recompra_pct` são LIFETIME de propósito — sem
-- filtro de `p_from`/`p_to` (recorrência é pergunta sobre a vida inteira do
-- cliente, não sobre um período; presas à janela do relatório ficariam
-- idênticas a `valor_medio_cents`). Só contam quem tem `won` de verdade:
-- cliente reconhecido na mão (Fase 3) sem venda registrada aqui não tem
-- valor pra somar.
create or replace function public.fn_sales_dashboard(
  p_org uuid,
  p_from timestamptz,
...
-- ---- fn_sales_dashboard: nome real do anúncio, não o headline genérico do clique (migration 0171) ----
-- Ver o cabeçalho da migration 0171: `ad_headline` (source_metadata) é igual
-- pra vários anúncios que compartilham a mesma copy — o rótulo "Anúncio" do
-- relatório precisa do nome real (`meta_ads_ad_metadata.ad_name`, já
-- resolvido pela Fase E), com o headline como fallback só quando a leitura
-- de campanha não está conectada.
create or replace function public.fn_sales_dashboard(
  p_org uuid,
  p_from timestamptz,
  p_to timestamptz,
...
-- ---- fn_sales_dashboard: lista de vendas do período, com nome do lead (migration 0173) ----
-- Ver o cabeçalho da migration 0173: o Dashboard de Vendas só tinha
-- agregado (KPIs, gráficos) — pedido ao vivo por uma LISTA com o nome de
-- cada lead vendido no período, respeitando os mesmos filtros do
-- cabeçalho do relatório. "Valor Orçado" não entra: o schema só guarda um
-- valor por negócio, não um "orçado" histórico distinto do final.
create or replace function public.fn_sales_dashboard(
  p_org uuid,
  p_from timestamptz,
  p_to timestamptz,
...
-- POSICIONADO ANTES DA VARREDURA DE ANON (migration 0116, logo abaixo) DE
-- PROPÓSITO: este apêndice cria `fn_due_appointment_reminders`, função
-- nova — e função criada DEPOIS da varredura nasceria exposta a `anon`
-- sem nada pra tirar esse acesso depois (regra vigiada por
-- `tests/unit/varredura-anon-e-o-ultimo-bloco.test.ts`). 0167/0168, que só
-- criam tabela e fazem UPDATE (nenhuma função), continuam depois da
-- varredura sem problema — só quem cria função precisa vir antes.
--
...
-- ---- campanhas: disparo de template aprovado pra uma lista de leads/contatos (migration 0167) ----
-- Ver o cabeçalho da migration 0167 pra contexto completo. Duas tabelas:
-- `whatsapp_campaigns` (a intenção) e `whatsapp_campaign_recipients` (o
-- público MATERIALIZADO na criação, não reavaliado a cada tick do
-- despachante).
