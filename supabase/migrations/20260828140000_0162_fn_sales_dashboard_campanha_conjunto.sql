-- 0162_fn_sales_dashboard_campanha_conjunto — Meta Ads, Fase E3: o
-- relatório "Receita por Anúncio" passa a trazer campanha, conjunto e
-- agendamentos por anúncio — não só receita isolada.
--
...
-- nenhuma chamada de rede aqui (função SQL não pode chamar Graph API, e nem
-- deveria: dashboard é leitura, não deve esperar por terceiro).
--
-- `receita_por_anuncio` MANTÉM o nome (não virou `desempenho_por_anuncio`):
-- é aditiva — o front-end e a rota já leem essa chave; trocar o nome só pra
-- refletir que agora carrega mais coisa seria quebra de contrato sem
-- ganho funcional. Os campos novos entram no MESMO objeto por linha; quem
-- não usa (Fase D antiga) ignora.
--
-- `agendamentos` por anúncio é dado NOVO — não existia quebrado por ad_id
-- antes (só existia como total da org em `funil_agendamento`, migration
-- 0158). Usa `EXISTS` correlacionado em vez de JOIN direto pra não
-- multiplicar linha de `base` por agendamento (um lead pode ter mais de um
-- `meeting_scheduled`, por remarcação — reagendar é emitir de novo, doutrina
-- da 0158) — sem o EXISTS, um lead remarcado 2x contaria como 2 leads na
-- métrica de leads/vendas do mesmo ad_id.
