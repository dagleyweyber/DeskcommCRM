-- 0171_fn_sales_dashboard_nome_real_do_anuncio — `receita_por_anuncio` usava
-- o texto genérico do clique no anúncio (`source_metadata->>'ad_headline'`,
-- o headline/CTA do referral do WhatsApp) como rótulo de "anúncio" — e esse
-- texto é IGUAL pra vários anúncios diferentes da mesma campanha (a Meta
-- manda o mesmo headline pra todas as variações de criativo que
-- compartilham a mesma copy). Achado ao vivo (RevitaFio Mossoró) depois de
-- conectar a leitura de campanha (0162/Fase E): toda linha da tabela
-- "Anúncio" aparecia com o MESMO texto ("Fale Conosco >>"), apesar de serem
-- anúncios distintos no Gerenciador de Anúncios (AD1, AD2, AD3...).
--
-- O nome REAL de cada anúncio já é resolvido e cacheado por
-- `meta_ads_ad_metadata.ad_name` (mesma Fase E que já resolve
-- campaign_name/adset_name, migration 0162) — a função já faz o LEFT JOIN
-- com essa tabela, só nunca tinha lido a coluna `ad_name` dela.
--
-- Fallback em cascata: nome real do anúncio → headline do clique (quando a
-- leitura de campanha não está conectada, recurso opcional) → ad_id cru
-- (nunca deixa a linha sem rótulo nenhum).
--
-- Base deste `create or replace` é a versão da 0166 (última a mexer nesta
-- função — exclusão de `existing_customer` + `ltv_medio_cents`/
-- `taxa_recompra_pct`), não a da 0162: `create or replace function`
-- substitui a definição INTEIRA, então partir de uma versão mais antiga
-- apagaria essas duas mudanças em silêncio.
